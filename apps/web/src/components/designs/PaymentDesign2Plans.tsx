...
function PlanCard({
  ...
  return (
    <Card className={`relative flex flex-col ${accent}`}>
      {isCurrent && (
        <div className="absolute -top-2.5 left-1/2 -translate-x-1/2">
          <Badge className="border-0 bg-foreground px-2 text-[10px] text-background">
            Current plan
          </Badge>
        </div>
      )}
      ...
      <div className="mx-auto max-w-4xl p-6">
        <div className="mb-6">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            You are on the <strong>{currentPlan}</strong> plan. Upgrade or
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          All prices in USD. Billed monthly. Cancel anytime. Annual billing
