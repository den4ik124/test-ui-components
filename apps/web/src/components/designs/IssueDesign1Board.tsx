...
const STATUS_CONFIG: Record<IssueStatus, { label: string; color: string; dot: string }> = {
  Created:    { label: "Open",        color: "bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/30 dark:text-blue-300",     dot: "bg-blue-500" },
  InProgress: { label: "In Progress", color: "bg-amber-100 text-amber-700 border-amber-200 dark:bg-amber-900/30 dark:text-amber-300", dot: "bg-amber-500" },
  Resolved:   { label: "Resolved",    color: "bg-emerald-100 text-emerald-700 border-emerald-200 dark:bg-emerald-900/30 dark:text-emerald-300", dot: "bg-emerald-500" },
};
    ...
    <Card className="mb-2 last:mb-0">
      <CardContent className="p-3">
        <div className="flex items-start gap-2 mb-2">
          <span className="text-base leading-none mt-0.5">{type.icon}</span>
          <p className="text-sm font-medium text-foreground leading-snug flex-1">{issue.title}</p>
        </div>
        {issue.description && (
        ...
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] text-muted-foreground">{formatRelative(issue.dateCreated)}</span>
        </div>
      </CardContent>
    </Card>
  );
  ...
  return (
    <div className="h-[calc(100svh-49px)] flex flex-col">
      <div className="px-6 py-4 border-b border-border shrink-0">
        ...
        <p className="text-sm text-muted-foreground">{dummyIssues.length} total issues</p>
      </div>
      <div className="flex-1 overflow-x-auto">
        <div className="flex gap-4 p-6 h-full min-w-[640px]">
